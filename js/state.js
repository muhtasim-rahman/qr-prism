...
const S = {
  ...
  // Patterns (IDs from designs/patterns.js, eye-frames.js, eye-inners.js)
  pattern: 'pat-square',
  eyeFrame: 'ef-square',
  eyeInner: 'ei-square',
  ...
  // Frame (ID from designs/frames.js)
  frame: 'frm-none',
  frameLabel: 'Scan Me',
  frameFont: 'Poppins',
  frameTSize: 100,
...
const SETTINGS = {
  ...
  accentColor: '#6C63FF',  // primary accent
  autoSaveProjects: true,  // auto save QR to projects on generate
  confirmDelete: true,
  defaultFormat: 'png',
  defaultSize: 600,
  defaultEC: 'H',
...
function loadSettings() {
  try {
    const saved = localStorage.getItem('qrs_settings');
...
function saveSettingsData() {
  try { localStorage.setItem('qrs_settings', JSON.stringify(SETTINGS)); } catch(e) {}
}
  ...
  try {
    switch (t) {
      ...
      case 'wifi': {
        ...
        if (!s) return '';
        const ssid = s.includes(';') || s.includes(',') || s.includes('"') || s.includes('\\') ? `"${s}"` : s;
        const pass = e !== 'nopass' ? `P:${p};` : 'P:;';
        const hid  = h ? 'H:true;' : '';
        return `WIFI:T:${e};S:${ssid};${pass}${hid};`;
      }
